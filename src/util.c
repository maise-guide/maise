...
void PRNT_HEAD()
{
  ...
  printf("|                     Hyperdimensional optimization                   |\n",VERSION);
  printf("|       https://github.com/maise-guide/maise/tree/hyper-maise.git     |\n");
  printf("=======================================================================\n");
}
//==============================================================================
...
void READ_MAIN(Tribe *T, ANN *R, PRS *P, Cell *C, int J, int ARGC)
{
  ...
  while( fgets(buf,200,in) )
  {
    ...
    if( strncmp(buf,"MNOT",4) == 0 ) { sscanf(buf+4,"%lf" ,&C->MNOT);       }
    if( strncmp(buf,"BETA",4) == 0 ) { sscanf(buf+4,"%lf" ,&C->BETA);       }
    if( strncmp(buf,"HMAP",4) == 0 ) { sscanf(buf+4,"%d"  ,&C->HMAP);       }
    if( strncmp(buf,"STEP",4) == 0 ) { sscanf(buf+4,"%d"  ,&C->STEP);       }
